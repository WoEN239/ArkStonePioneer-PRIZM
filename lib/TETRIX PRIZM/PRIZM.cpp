...
#define IIC_TRANSMISSON_DELAY 10
#define INTEGRATED_DCEXP_ADDR 5
#define INT_SVOEXP_ADDR 6
...
void PRIZM::setMotorSpeedPID(int P, int I, int D) {    //=== Change the speed PID parameters for DC Chip
    ...
    expansion.setMotorSpeedPID(INTEGRATED_DCEXP_ADDR, P, I, D);
...
int PRIZM::readDCFirmware() {      //==== Request firmware version of DC motor chip
    ...
    return expansion.readDCFirmware(INTEGRATED_DCEXP_ADDR);

}

int PRIZM::readSVOFirmware() {        //==== Request firmware version of Servo motor chip

    return expansion.readSVOFirmware(INT_SVOEXP_ADDR);
...
void PRIZM::PrizmBegin() {  //======= Send a SW reset to all EXPANSIONansion port I2C
    ...
    Wire.beginTransmission(INTEGRATED_DCEXP_ADDR);            // Supported I2C addresses for EXPANSIONansion controllers is 1 - 4 (4 boxes total)
    ...
    delay(IIC_TRANSMISSON_DELAY);
    Wire.beginTransmission(INT_SVOEXP_ADDR);
    Wire.write(0x27);
    Wire.endTransmission();
    delay(IIC_TRANSMISSON_DELAY);
    ...
    delay(IIC_TRANSMISSON_DELAY);                            // No guarantee that more than 4 boxes will work on single I2C bus because of cable capacitance
    Wire.beginTransmission(INT_SVOEXP_ADDR);
    Wire.write(0x27);
    Wire.endTransmission();
    delay(IIC_TRANSMISSON_DELAY);
...
void PRIZM::PrizmEnd() {  //======= Send a SW reset to all I2C devices(resets everything) This is done mainly to stop all motors
    ...
    Wire.beginTransmission(INTEGRATED_DCEXP_ADDR);            // Supported I2C addresses for EXPANSIONansion controllers is 1 - 4
...
void EXPANSION::setServoSpeed(int address, int channel, int servospeed) {   //=========== function for setting servo speeds individually for  servo EXPANSIONANSION
    ...
    if (channel >= 1 && channel <= 6) { channel = NSERVO_BYTE+channel-1; }
...
void PRIZM::setServoSpeed(int channel, int servospeed) {   //=========== function for setting PRIZM servo speeds individually
    ...
    expansion.setServoSpeed(INT_SVOEXP_ADDR, channel, servospeed);

}

void PRIZM::setServoSpeeds(int servospeed1, int servospeed2, int servospeed3, int servospeed4, int servospeed5,
                           int servospeed6) {   // function to set all PRIZM servo speeds at once
...
void EXPANSION::setServoSpeeds(int address, int servospeed1, int servospeed2, int servospeed3, int servospeed4, int servospeed5,
                               int servospeed6) {   // function to set all EXPANSIONANSION servo speeds at once
 ...
 */
    ...
    if (xmit == 1) {                    // no need to send if positions have not changed - result frees up I2C bus
        Wire.beginTransmission(
                INT_SVOEXP_ADDR);        // Even though this is a single position command, sending all channels at once works around minor servo glitching in version 1 PRIZM servo chip firmware.
        Wire.write(0x35);
...
void PRIZM::setServoPositions(int servoposition1, int servoposition2, int servoposition3, int servoposition4, int servoposition5,
                              int servoposition6) {  // Sets all PRIZM servo positions at once
...
void EXPANSION::setServoPositions(int address, int servoposition1, int servoposition2, int servoposition3, int servoposition4, int servoposition5,
                                  int servoposition6) {  // Sets all EXPANSIONANSION servo positions at once
...
void EXPANSION::setMotorSpeed(int address, int channel,
                              long Mspeed) {      // === set speed of each EXPANSIONANSION DC motor == requires a 1440 CPR installed encoder to do the PID
...
void EXPANSION::setMotorSpeeds(int address, long Mspeed1,
                               long Mspeed2) {      // === BLOCK write to set speeds of both EXPANSIONANSION motors at once == 1440 CPR encoder must be installed to do PID
...
void PRIZM::setMotorTarget(int channel, long Mspeed,
                           long Mtarget) {      // === set speed and encoder target of each PRIZM DC motor == requires a 1440 CPR encoder to do the PID
...
void EXPANSION::setMotorTarget(int address, int channel, long Mspeed,
                               long Mtarget) {      // === set speed and encoder target of each EXPANSIONANSION DC motor == requires a 1440 CPR encoder to do the PID
...
void PRIZM::setMotorTargets(long Mspeed1, long Mtarget1, long Mspeed2,
                            long Mtarget2) {      // === BLOCK WRITE set speed and encoder target of both PRIZM DC motors == requires a 1440 CPR encoder to do the PID
...
void EXPANSION::setMotorTargets(int address, long Mspeed1, long Mtarget1, long Mspeed2,
                                long Mtarget2) {      // === BLOCK WRITE set speed and encoder target of both EXPANSIONANSION DC motors == requires a 1440 CPR encoder to do the PID
...
void PRIZM::setMotorDegree(int channel, long Mspeed,
                           long Mdegrees) {      // === set speed and encoder target of each PRIZM DC motor in DEGREES  == requires a 1440 CPR encoder to do the PID
...
void EXPANSION::setMotorDegree(int address, int channel, long Mspeed,
                               long Mdegrees) {      // === set speed and encoder target of each EXPANSIONANSION DC motor in DEGREES  == requires a 1440 CPR encoder to do the PID
...
void PRIZM::setMotorDegrees(long Mspeed1, long Mdegrees1, long Mspeed2,
                            long Mdegrees2) {      // === BLOCK WRITE set speed and encoder target in DEGREES of both PRIZM DC motors == requires a 1440 CPR encoder to do the PID
...
void EXPANSION::setMotorDegrees(int address, long Mspeed1, long Mdegrees1, long Mspeed2,
                                long Mdegrees2) {      // === BLOCK WRITE set speed and encoder target in DEGREES of both EXPANSIONANSION DC motors == requires a 1440 CPR encoder to do the PID
...
long EXPANSION::readEncoderCount(int address,
                                 int channel) {   // ============================= READ EXPANSIONANSION ENCODER DATA COUNTS ====================================
...
long EXPANSION::readEncoderDegrees(int address,
                                   int channel) {   // ============================= READ EXPANSIONANSION ENCODER DATA DEGREES ====================================
...
int EXPANSION::readMotorBusy(int address,
                             int channel) {            // ================== Read Busy Status of EXPANSIONANSION DC motor channel =======================
...
int
EXPANSION::readMotorCurrent(int address, int channel) {        // ================ Read DC motor current of EXPANSIONANSION DC channels ======================
...
void PRIZM::setMotorInvert(int channel,
                           int invert) {                        // ======================== Set the PRIZM DC Motor Direction invert status ====================
...
void EXPANSION::setMotorInvert(int address, int channel,
                               int invert) {            // ======================== Set the EXPANSIONANSION DC Motor Direction invert status ====================
