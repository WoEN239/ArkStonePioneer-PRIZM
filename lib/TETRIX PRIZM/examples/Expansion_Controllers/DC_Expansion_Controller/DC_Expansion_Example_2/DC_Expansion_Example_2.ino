...
void loop() {
  ...
  exc.setMotorTarget(1,1,200,1440);       // Spin motor 1 at 200 degrees per second to encoder count value target 1440.
                                          // Once on target, the motor will hold position in a servo-like mode.
                                          // When using the expansion controllers, the first number (1) in the function parameter is the controller's ID#/I2C address.

  delay(5000);                            // wait here for 5 seconds. Motor should be at encoder count target 1440 and holding position.

  exc.setMotorSpeed(1,1,200);             // Spin motor 1 at a constant rate of 200 degrees per second.
